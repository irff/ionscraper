...
class HuffingtonpostSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(SgmlLinkExtractor(
            allow=('(com/)\d{2}[0-9]','(com/)\w{2}[0-9a-zA-Z]','/'),
    ...
    def parse_item(self, response):
        ...
        news['provider'] = "huffingtonpost.com"
    ...
    def parse_item_default(self, response, news):
        ...
        news['content'] = helper.item_merge(response.xpath("//div[@class='content']").extract())
        news['title'] = helper.clear_item(news['title'])
        news['content'] = helper.clear_item(news['content'])

        author = response.xpath("//span[@class='name fn']/text()").extract()
        if len(author) > 0:
        ...
        if len(date) >= 5:
            news['publish'] = self.huffingtonpost_date(date)
        else:
    ...
    def huffingtonpost_date(self, date_string):
